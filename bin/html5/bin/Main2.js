...
ApplicationMain.main = function() {
	...
	ApplicationMain.loaders.set("graphics/caveparallax.png",loader9);
	ApplicationMain.total++;
	var loader10 = new flash.display.Loader();
	ApplicationMain.loaders.set("graphics/player.png",loader10);
	ApplicationMain.total++;
	var loader11 = new flash.display.Loader();
	ApplicationMain.loaders.set("graphics/visibonepallet.png",loader11);
	ApplicationMain.total++;
	var urlLoader = new flash.net.URLLoader();
	urlLoader.set_dataFormat(flash.net.URLLoaderDataFormat.BINARY);
	ApplicationMain.urlLoaders.set("maps/testCave.tmx",urlLoader);
	ApplicationMain.total++;
	...
	if(ApplicationMain.total == 0) ApplicationMain.begin(); else {
		...
		while( $it1.hasNext() ) {
			var path1 = $it1.next();
			var urlLoader1 = ApplicationMain.urlLoaders.get(path1);
			urlLoader1.addEventListener("complete",ApplicationMain.loader_onComplete);
			urlLoader1.load(new flash.net.URLRequest(path1));
		}
	}
};
...
Main.prototype = $extend(com.haxepunk.Engine.prototype,{
	...
	,resize: function() {
		com.haxepunk.HXP.screen.set_scaleX(com.haxepunk.HXP.screen.set_scaleY(16));
		com.haxepunk.HXP.resize(com.haxepunk.HXP.stage.get_stageWidth(),com.haxepunk.HXP.stage.get_stageHeight());
	}
	,__class__: Main
});
...
var DefaultAssetLibrary = function() {
	...
	this.addExternal("graphics/caveparallax.png","image","graphics/caveparallax.png");
	this.addExternal("graphics/player.png","image","graphics/player.png");
	this.addExternal("graphics/visibonepallet.png","image","graphics/visibonepallet.png");
	this.addEmbed("font/04B_03__.ttf","font",__ASSET__font_5);
	this.addExternal("maps/testCave.tmx","binary","maps/testCave.tmx");
};
...
$hxClasses["GlobalVariables"] = GlobalVariables;
GlobalVariables.__name__ = ["GlobalVariables"];
...
var MainScene = function() {
	...
	com.haxepunk.HXP._scene.camera.x = 0;
	com.haxepunk.HXP._scene.camera.y = -16.;
	this.createMap();
	this.player = new entities.Player(48.,32.);
...
MainScene.prototype = $extend(com.haxepunk.Scene.prototype,{
	createMap: function() {
		var b = new com.haxepunk.graphics.Backdrop("graphics/caveparallax.png",true,true);
		b.x = 32;
		b.y = 16;
		b.scrollX = 0.1;
		b.scrollY = 0.1;
		...
		e.set_layer(99);
		this.add(e);
		var e1 = new com.haxepunk.tmx.TmxEntity((function($this) {
			var $r;
			var map = new com.haxepunk.tmx.TmxMap((function($this) {
				var $r;
				var xml = Xml.parse(openfl.Assets.getText("maps/testCave.tmx"));
		...
		e1.y = 16;
		e1.set_layer(10);
		e1.loadGraphic("graphics/visibonepallet.png",["collidible","non-collidible"]);
		e1.loadMask("collidible","terrain");
		this.add(e1);
	}
	,update: function() {
		this.add(this.player);
...
com.haxepunk.graphics.Canvas.prototype = $extend(com.haxepunk.Graphic.prototype,{
			...
			while(xx < this._refWidth) {
				...
				if(this.angle == 0 && this.blend == null) {
					if(sx == 1 && sy == 1 && this._tint == null) {
						if(buffer.___textureBuffer != null) this._rect.width = buffer.___textureBuffer.width; else this._rect.width = 0;
						if(buffer.___textureBuffer != null) this._rect.height = buffer.___textureBuffer.height; else this._rect.height = 0;
						target.copyPixels(buffer,this._rect,this._point,null,null,true);
					} else {
						var i = Std["int"](this._ref.getPixel(xx,yy));
						var w;
						w = (buffer.___textureBuffer != null?buffer.___textureBuffer.width:0) * sx | 0;
						var h;
						h = (buffer.___textureBuffer != null?buffer.___textureBuffer.height:0) * sy | 0;
						var wrongSize = i >= this._midBuffers.length || this._midBuffers[i].get_width() != w || this._midBuffers[i].get_height() != h;
						if(this._redrawBuffers || wrongSize) {
							if(wrongSize) {
								if(i < this._midBuffers.length) this._midBuffers[i].dispose();
								this._midBuffers[i] = com.haxepunk.HXP.createBitmap(w,h,true);
					...
					var _g1 = this._matrix;
					_g1.set_ty(_g1.ty + this._point.y);
	...
	,drawRect: function(rect,color,alpha) {
		...
		while(_g2 < _g11.length) {
			...
			buffer2.draw(com.haxepunk.HXP.sprite);
			xx += this._maxWidth;
			if(xx >= this._width) {
				xx = 0;
				yy += this._maxHeight;
	...
	,set_color: function(value) {
		...
		this._color = value;
		this._red = com.haxepunk.HXP.getRed(this.get_color()) / 255;
		this._green = com.haxepunk.HXP.getGreen(this.get_color()) / 255;
		this._blue = com.haxepunk.HXP.getBlue(this.get_color()) / 255;
	...
	,__properties__: $extend(com.haxepunk.Graphic.prototype.__properties__,{get_height:"get_height",get_width:"get_width",set_alpha:"set_alpha",get_alpha:"get_alpha",set_color:"set_color",get_color:"get_color"})
});
com.haxepunk.graphics.Backdrop = function(source,repeatX,repeatY) {
	...
	if(js.Boot.__instanceof(source,com.haxepunk.graphics.atlas.AtlasRegion)) this.setAtlasRegion(source); else if(com.haxepunk.HXP.renderMode == com.haxepunk.RenderMode.HARDWARE) this.setAtlasRegion(com.haxepunk.graphics.atlas.Atlas.loadImageAsRegion(source)); else {
		if(js.Boot.__instanceof(source,flash.display.BitmapData)) this.setBitmapSource(source); else if(js.Boot.__instanceof(source,Dynamic)) this.setBitmapSource(com.haxepunk.HXP.getBitmap(source));
		if(this._source == null && this._region == null) this.setBitmapSource(com.haxepunk.HXP.createBitmap(com.haxepunk.HXP.width,com.haxepunk.HXP.height,true));
	}
...
com.haxepunk.graphics.Backdrop.prototype = $extend(com.haxepunk.graphics.Canvas.prototype,{
	setAtlasRegion: function(region) {
		this.blit = false;
		this._region = region;
		this._textWidth = region._rect.width | 0;
		this._textHeight = region._rect.height | 0;
	}
	...
	,renderAtlas: function(layer,point,camera) {
		...
		var px = this._point.x * fsx | 0;
		var py = this._point.y * fsy | 0;
		var y = 0;
		while(y < this._height * sy * fsy) {
			var x = 0;
			while(x < this._width * sx * fsx) {
				this._region.draw(px + x,py + y,layer,sx * fsx,sy * fsy,0,this._red,this._green,this._blue,this._alpha);
				x += this._textWidth * fsx | 0;
			}
			y += this._textHeight * fsy | 0;
		}
	}
...
entities.Player = function(x,y) {
	this.playerLeftLegColor = 6710886;
	this.playerRightLegColor = 6710886;
	this.playerLeftArmColor = 6710886;
	this.playerRightArmColor = 6710886;
	this.playerArmorLowerColor = 8355711;
	this.playerArmorUpperColor = 8355711;
	this.playerHelmetColor = 6710886;
	this.playerJumpHeight = 13;
	this.playerMaxMana = 10;
	this.playerMaxHealth = 10;
	this.playerJumping = false;
	this.playerJumpingCounter = 0;
	...
	this.set_layer(10);
	this.drawPlayer();
	this.ui = new entities.UI(32,40);
	this.userInput = new utilities.InputHandler();
...
entities.Player.prototype = $extend(com.haxepunk.Entity.prototype,{
	handleInput: function() {
		...
		if(this.userInput.inputRight) this.velocityX = 1;
		if(this.userInput.inputLeft) this.velocityX = -1;
		if(this.userInput.inputUp) this.velocityY = -2;
		if(this.userInput.inputDown) this.velocityY = 1;
	}
	,drawPlayer: function() {
		this.playerHelmet = com.haxepunk.graphics.Image.createRect(1,1,this.playerHelmetColor);
		this.playerHelmet.x = 1;
		this.playerHelmet.y = 0;
		this.addGraphic(this.playerHelmet);
		this.playerArmorUpper = com.haxepunk.graphics.Image.createRect(1,1,this.playerArmorUpperColor);
		this.playerArmorUpper.x = 1;
		this.playerArmorUpper.y = 1;
		this.addGraphic(this.playerArmorUpper);
		this.playerArmorLower = com.haxepunk.graphics.Image.createRect(1,1,this.playerArmorLowerColor);
		this.playerArmorLower.x = 1;
		this.playerArmorLower.y = 2;
		this.addGraphic(this.playerArmorLower);
		this.playerLeftArm = com.haxepunk.graphics.Image.createRect(1,1,this.playerLeftArmColor);
		this.playerLeftArm.x = 0;
		this.playerLeftArm.y = 1;
		this.addGraphic(this.playerLeftArm);
		this.playerRightArm = com.haxepunk.graphics.Image.createRect(1,1,this.playerRightArmColor);
		this.playerRightArm.x = 2;
		this.playerRightArm.y = 1;
		this.addGraphic(this.playerRightArm);
		this.playerLeftLeg = com.haxepunk.graphics.Image.createRect(1,1,this.playerLeftLegColor);
		this.playerLeftLeg.x = 0;
		this.playerLeftLeg.y = 3;
		this.addGraphic(this.playerLeftLeg);
		this.playerRightLeg = com.haxepunk.graphics.Image.createRect(1,1,this.playerRightLegColor);
		this.playerRightLeg.x = 2;
		this.playerRightLeg.y = 3;
		this.addGraphic(this.playerRightLeg);
	}
	,move: function() {
		if(this.playerJumpingCounter <= 0 && this.playerJumping == false && this.userInput.inputUp) {
			this.playerJumping = true;
			this.playerJumpingCounter = this.playerJumpHeight;
		} else if(this.playerJumpingCounter > 0 && this.playerJumping == true) this.playerJumpingCounter--; else if(this.playerJumpingCounter <= 0) this.velocityY = 0;
		if(this.currentMoveDelay <= 0) {
			...
			if(e == null) {
				...
				var _g = this.ui;
				_g.x = (_g.followCamera?_g.x + com.haxepunk.HXP.camera.x:_g.x) + this.velocityX;
				this.currentMoveDelay = 3;
			} else {
				if(this.velocityY == 0) this.velocityY = -2;
				haxe.Log.trace("Colliding with terrain on X",{ fileName : "Player.hx", lineNumber : 137, className : "entities.Player", methodName : "move"});
				this.moveBy(0,0);
			}
			var e1 = this.collide("terrain",this.followCamera?this.x + com.haxepunk.HXP.camera.x:this.x,(this.followCamera?this.y + com.haxepunk.HXP.camera.y:this.y) + this.velocityY + 1);
			if(e1 == null) {
				this.moveBy(0,this.velocityY + 1);
				this.currentMoveDelay = 3;
			} else {
				haxe.Log.trace("Colliding with terrain on Y",{ fileName : "Player.hx", lineNumber : 153, className : "entities.Player", methodName : "move"});
				this.moveBy(0,0);
				if(this.velocityY >= 0) {
					this.playerJumping = false;
					this.playerJumpingCounter = 0;
...
entities.UI = function(x,y) {
	this.uiBoxOffsetFromX = 0;
	this.uiBoxBackgroundColor = 0;
	this.uiBoxBuffer = 1;
	this.uiHealthManaBarWidth = 1;
	this.uiHealthManaBarHeight = 6;
	this.uiBoxWidth = 6;
	com.haxepunk.Entity.call(this,x,y);
	this.width = 32;
	this.height = 8;
	...
	this.set_layer(5);
	this.set_type("ui");
	this.uiBackground = com.haxepunk.graphics.Image.createRect(32,8,3355443);
	this.addGraphic(this.uiBackground);
	this.uiBox1 = com.haxepunk.graphics.Image.createRect(this.uiBoxWidth,this.uiBoxWidth,this.uiBoxBackgroundColor);
	this.uiBox1.x = this.uiBackground.x + this.uiBoxWidth * 0 + this.uiBoxBuffer;
	this.uiBox1.y = this.uiBackground.y + this.uiBoxBuffer;
	this.addGraphic(this.uiBox1);
	this.uiBox2 = com.haxepunk.graphics.Image.createRect(this.uiBoxWidth,this.uiBoxWidth,this.uiBoxBackgroundColor);
	this.uiBox2.x = this.uiBoxOffsetFromX + this.uiBackground.x + this.uiBoxWidth + this.uiBoxBuffer * 2;
	this.uiBox2.y = this.uiBackground.y + this.uiBoxBuffer;
	this.addGraphic(this.uiBox2);
	this.uiBox3 = com.haxepunk.graphics.Image.createRect(this.uiBoxWidth,this.uiBoxWidth,this.uiBoxBackgroundColor);
	this.uiBox3.x = 2 * this.uiBoxOffsetFromX + this.uiBackground.x + this.uiBoxWidth * 2 + this.uiBoxBuffer * 3;
	this.uiBox3.y = this.uiBackground.y + this.uiBoxBuffer;
	this.addGraphic(this.uiBox3);
	this.uiBox4 = com.haxepunk.graphics.Image.createRect(this.uiBoxWidth,this.uiBoxWidth,this.uiBoxBackgroundColor);
	this.uiBox4.x = 3 * this.uiBoxOffsetFromX + this.uiBackground.x + this.uiBoxWidth * 3 + this.uiBoxBuffer * 4;
	this.uiBox4.y = this.uiBackground.y + this.uiBoxBuffer;
	this.addGraphic(this.uiBox4);
	this.uiHealthManaBar = com.haxepunk.graphics.Image.createRect(2 * this.uiHealthManaBarWidth,this.uiHealthManaBarHeight,this.uiBoxBackgroundColor);
	this.uiHealthManaBar.x = 4 * this.uiBoxOffsetFromX + this.uiBackground.x + this.uiBoxWidth * 4 + this.uiBoxBuffer * 5;
	this.uiHealthManaBar.y = this.uiBackground.y + this.uiBoxBuffer;
	this.addGraphic(this.uiHealthManaBar);
};
$hxClasses["entities.UI"] = entities.UI;
entities.UI.__name__ = ["entities","UI"];
entities.UI.__super__ = com.haxepunk.Entity;
entities.UI.prototype = $extend(com.haxepunk.Entity.prototype,{
	__class__: entities.UI
});
flash._Lib = {};
...
var utilities = {};
utilities.InputHandler = function() {
...
$hxClasses["utilities.InputHandler"] = utilities.InputHandler;
utilities.InputHandler.__name__ = ["utilities","InputHandler"];
utilities.InputHandler.prototype = {
	update: function() {
		if(com.haxepunk.utils.Input.check("up")) this.inputUp = true; else this.inputUp = false;
		if(com.haxepunk.utils.Input.check("down")) this.inputDown = true; else this.inputDown = false;
		if(com.haxepunk.utils.Input.check("left")) this.inputLeft = true; else this.inputLeft = false;
		if(com.haxepunk.utils.Input.check("right")) this.inputRight = true; else this.inputRight = false;
	}
	,__class__: utilities.InputHandler
};
...
GlobalVariables.SCREEN_SIZE_X = 32;
GlobalVariables.SCREEN_SIZE_Y = 32;
GlobalVariables.SCREEN_SCALE = 16;
GlobalVariables.SCREEN_POS_X = 32;
GlobalVariables.SCREEN_POS_Y = 16;
GlobalVariables.UI_X = 0;
GlobalVariables.UI_Y = 24;
GlobalVariables.GRAVITY = 1;
GlobalVariables.DEFAULT_LAYER = 10;
GlobalVariables.PLAYER_LAYER = 8;
GlobalVariables.BACKGROUND_LAYER = 99;
GlobalVariables.PLAYER_SIZE = 1;
...
entities.Player.maxMoveDelay = 3;
entities.UI.UI_HEIGHT = 8;
entities.UI.UI_WIDTH = 32;
entities.UI.UI_BACKGROUND_COLOR = 3355443;
entities.UI.UI_LAYER = 5;
